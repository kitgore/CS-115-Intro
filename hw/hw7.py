...
class Term:
    ...
    def __call__(self, input): # myTerm(input)
        ...
        return (self.coef * pow(input, self.exp))
...
# exponent is strictly decreasing
    ...
    def __call__(self, input): # myPoly(input)
        ...
        sum = 0
        for i in range(len(self.polyList)):
            sum += (self.polyList[i].coef * pow(input, self.polyList[i].exp))
        return sum
    ...
    def __neg__(self): # negPoly = -myPoly
        ...
        myPoly = LinkedPolynomial(self.polyList)
        for i in range(len(self.polyList)):
            myPoly.polyList[i] = -self.polyList[i]
        return myPoly
    ...
    def __mul__(self, otherPoly): # mulPoly = myPoly * otherPoly
        ...
        finalPoly = LinkedPolynomial([])
        for i in range(len(self.polyList)):
            for j in range(len(otherPoly.polyList)):
                newTerm = Term((self.polyList[i].coef * otherPoly.polyList[j].coef), (self.polyList[i].exp + otherPoly.polyList[j].exp))
                finalPoly += LinkedPolynomial([newTerm])
        return finalPoly